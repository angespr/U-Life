...
function SavedJobModal({ onClose, onCreate }) {
  ...
  const submit = (e) => {
    ...
    onCreate({
      ...
      location: location.trim() || "Not listed",
      status,
    });
...
function Opportunity() {
  ...
  const [showSavedJobModal, setShowSavedJobModal] = useState(false);
  ...
  const [saved, setSaved] = useState(
    data.savedOpportunities || defaultSavedJobs
  );
  ...
  const saveSavedJobs = (updatedJobs) => {
    setSaved(updatedJobs);
    saveData({
      ...data,
      savedOpportunities: updatedJobs,
    });
  };

  const addSavedOpportunity = (job) => {
    saveSavedJobs([...saved, job]);
  };

  const deleteSavedOpportunity = (id) => {
    saveSavedJobs(saved.filter((item) => item.id !== id));
  };
  ...
  const getNetworkEvents = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/api/network-events/search`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    <PageStyle>
      ...
      <section className="quick-actions">
        ...
        <div className="quick-action-grid">
          ...
          <button
            className="quick-action-card clickable-card"
            onClick={() => setShowNetworkEvents((prev) => !prev)}
          >
        ...
        <section className="feature-card full-width network-events-panel">
          <div className="section-header-row">
            <div>
              ...
              <p>
                Search for career fairs, meetups, conferences, and professional
                networking events.
              </p>
            </div>
          </div>
          ...
          {networkEvents.length > 0 && (
            <div className="network-events-list">
              {networkEvents.map((event, index) => (
                <div key={index} className="network-event-card">
                  <div>
                    <h3>{event.name || event.title || "Networking Event"}</h3>

                    <p>
                      <strong>Date:</strong>{" "}
                      {event.date || event.time || "Not listed"}
                    </p>
      ...
      <section className="feature-card full-width">
        <div className="section-header-row">
          <div>
            ...
            <p>Track saved jobs, applications, and interview opportunities.</p>
          </div>

          <button
            className="small-add-btn"
            onClick={() => setShowSavedJobModal(true)}
          >
            + Add Job
          </button>
        </div>
            ...
            <div key={item.id} className="saved-row">
              ...
              <div className="saved-actions">
                ...
                <button
                  ...
                  onClick={() => deleteSavedOpportunity(item.id)}
                >
                  ×
                </button>
              </div>
      ...
      {showSavedJobModal && (
        <SavedJobModal
          onClose={() => setShowSavedJobModal(false)}
          onCreate={addSavedOpportunity}
        />
      )}
    </PageStyle>
  );
